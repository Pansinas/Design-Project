...
# Load bank statement data (CSV format for now)
...
# Predict next month’s expenses using an improved model
def predict_future_expense(df):
    ...
    model.fit(monthly_expenses[["Month_Num"]], monthly_expenses["Amount"])
    next_month = [[len(monthly_expenses) + 1]]
    future_expense = model.predict(next_month)[0]
    print(f"Predicted Expense for Next Month: ₹{future_expense:.2f}")
    
# Main execution
def main():
    file_path = "bank_statement.pdf"  # Replace with actual file path
    df = load_data(file_path)
    print(df.head())  # Display first few records
    plot_spending_pie_chart(df)
    predict_future_expense(df)
